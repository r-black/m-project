...
class Command(BaseCommand):
    help = 'Heart disease risk prediction training'
    ...
    def handle(self, *args, **options):
        ...
        def plot_confusion_matrix(cm, classes,
            ...
            plt.grid(None)
            plt.tight_layout()
            plt.ylabel('Истинные метки', size = 14)
            plt.xlabel('Спрогнозированные метки', size = 14)

        # Confusion matrix
        y_pred = model.predict(X_test)
        cm = confusion_matrix(y_test, y_pred)
        plot_confusion_matrix(cm, classes = ['Нет риска', 'Есть риск'],
                              title = 'Матрица погрешностей оценки рисков')
        ...
        # ax = plt.gca()
        # rfc_disp = plot_roc_curve(model, X_train, y_train, ax=ax, alpha=0.8)
